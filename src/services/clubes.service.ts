...
export async function getProjectsByClub(id_club: string){
    let { data, error } = await supabase()
    .rpc('get_projects_by_club', { id_club });
    if (error) throw new Error('Error al intentar obtener los proyectos asignados al club seleccionado. Intente de nuevo. Error: ' + error.message);
    return data as [];
}
