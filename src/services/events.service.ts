...
export async function addEvent(event: Event): Promise<boolean>{
    const {nombre, descripcion, fecha_inicio, fecha_fin, responsable, creado_por} = event;
...
export async function updateEvent(event: Event): Promise<boolean> {
    const {id, nombre, descripcion, fecha_inicio, fecha_fin, responsable, estado, actualizado_por} = event;
    let { error } = await supabase()
        .from('eventos')
        .update({
            nombre,
        ...
        .select();
    if(error){
        throw new Error('Ha ocurrido un error al intentar actualizar el evento. Intente de nuevo.');
    }
    return true;
}
...
export async function getEvents(){
    ...
    .rpc('get_events')
    if (error) throw new Error('Error al intentar recuperar todos los clubes disponibles. Rceargue la página e intente de nuevo.');
    return data as [];
}

export async function deleteEvent(event: Event, status: string): Promise<boolean>{
    ...
    let { error } = await supabase()
        ...
        .select();
    if(error){
        throw new Error('Ha ocurrido un error al intentar eliminar el evento. Intente de nuevo.');
    }
    return true;
}
