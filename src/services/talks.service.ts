...
export async function addTalk(talk: Talk): Promise<boolean> {
    const {nombre, descripcion, id_evento,fecha, lugar, hora_inicio, hora_fin, creado_por} = talk;
...
export async function addTalkWithSpeakers(talk: Talk): Promise<boolean>{
    const {nombre, descripcion, id_evento, fecha, lugar, hora_inicio, hora_fin, creado_por, ponentes, observaciones, precios} = talk;
...
export async function updateTalk(talk: Talk): Promise<boolean> {
    ...
    let { error } = await supabase()
        ...
        .eq('id', id)
        .select();
    if (error) throw new Error('Error al intentar actualizar la charla seleccionada. Intente de nuevo. Error: ' + error.message);
    return true;
}
...
export async function getTalks(){
    let { data, error } = await supabase()
    .rpc('get_talks');
    if (error) throw new Error('Error al intentar obtener las charlas. Intente de nuevo. Error: ' + error.message);
    return data as [];
}
...
export async function getTalksByEventId(evento_id: string){
    ...
    if (error) throw new Error('Error al intentar obtener las charlas del evento seleccionado. Intente de nuevo. Error: ' + error.message);
    return data as [];
}
...
export async function deleteTalk(talk: Talk, estado: string): Promise<boolean> {
    ...
    let { error } = await supabase()
        ...
        .eq('id', id)
        .select();
    if ( error ) throw new Error('Error al intentar eliminar la charla seleccionada. Intente de nuevo. Error: ' + error.message);
    return true;
}